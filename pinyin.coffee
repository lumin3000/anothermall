exports.full = getPinyin = (words)->
  wordsArray = []
  words.split("").forEach (char)->wordsArray.push getChar char
  wordsArray.forEach (char,idx)->
    console.log('processing:    {'+char.pinyin+'}   :'+char.english+';'+words)
    if idx>0 && !char.english #only add a " " to chinese pinyin
      wordsArray[idx]=" "+char.pinyin
    else
      wordsArray[idx]=char.pinyin
  wordsArray.join ""    

exports.short = getPinyinShort = (words)->
  wordsArray = []
  words.split("").forEach (char)->wordsArray.push getChar(char).pinyin.substr(0,1)
  wordsArray.join ""

getChar = (char)->
  res = {pinyin:'*',english:false}
  if char.charAt(0)<='~'
    return {pinyin:char,english:true}
  Object.keys(pinyin_data).forEach (key)->
    return res.pinyin=key if pinyin_data[key].indexOf(char) >=0
  return res
